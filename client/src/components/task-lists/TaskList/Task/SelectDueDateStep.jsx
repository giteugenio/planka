...
import styles from '../../../cards/EditDueDateStep/EditDueDateStep.module.scss';

const SelectDueDateStep = React.memo(({ taskId, onBack, onClose }) => {
  const selectTaskById = useMemo(() => selectors.makeSelectTaskById(), []);

  const defaultValue = useSelector((state) => {
    const task = selectTaskById(state, taskId);
    return task && task.dueDate ? new Date(task.dueDate) : null;
  });
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!defaultValue || value.getTime() !== new Date(defaultValue).getTime()) {
      dispatch(
...
SelectDueDateStep.propTypes = {
  taskId: PropTypes.string.isRequired,
  onBack: PropTypes.func,
  onClose: PropTypes.func.isRequired,
};

SelectDueDateStep.defaultProps = {
  onBack: undefined,
};

export default SelectDueDateStep;
